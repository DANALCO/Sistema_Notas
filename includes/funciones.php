...
function promedio($alumno){
    global $pdo;
    $promedio =0;
    $sqlCanNotas = "SELECT COUNT(valor_nota) as numero FROM notas as n INNER JOIN ev_entregadas as ev ON n.ev_entregada_id = ev.ev_entregada_id WHERE ev.alumno_id = $alumno";
    $querryCanNotas = $pdo->prepare($sqlCanNotas);
    $querryCanNotas->execute();
    
    if($row = $querryCanNotas->fetch()){
